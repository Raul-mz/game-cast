...
class dbHelper {
    ...
    function __construct() {
        ...
        try {
            ...
            $response["message"] = 'Conneccion fallida: ' . $e->getMessage();
            $response["data"] = null;
    ...
    function selectSql($table){
     ...
     function select2Table($table1,$table2, $columns, $where, $columnID1,$columnID2){
        try{
            ...
            $stmt = $this->db->prepare("select ".$columns." from ".$table1.",".$table2." where ".$table1.".".$columnID1."=".$table2.".".$columnID2." and 1=1 ". $w);
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
    ...
    function select2($table, $columns, $where, $order){
        try{
            ...
            if(count($rows)<=0){
                ...
                $response["status"] = "success";
                $response["message"] = "Datos seleccionados de la base de datos";
            }
                $response["data"] = $rows;
        }catch(PDOException $e){
            $response["status"] = "error";
